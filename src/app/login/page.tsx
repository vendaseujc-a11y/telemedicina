...
function LoginForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (lockedUntil && Date.now() < lockedUntil) {
      ...
      setError(`Conta bloqueada. Tente novamente em ${minutes} minuto(s).`);
      setLoading(false);
      return;
    }
      ...
      <main className="flex-1 flex items-center justify-center px-4 pb-8">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold text-zinc-900 mb-2">Bem-vindo de volta</h1>
            <p className="text-zinc-500">Entre com sua conta para continuar</p>
          </div>

          {registered && (
            <div className="bg-emerald-50 text-emerald-700 px-4 py-3 rounded-xl mb-6 text-sm">
          ...
          {warning && (
            <div className="bg-amber-50 text-amber-700 px-4 py-3 rounded-xl mb-4 text-sm flex items-center gap-2">
              <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {attemptsRemaining !== null && attemptsRemaining <= 3 && attemptsRemaining > 0 && !lockedUntil && (
            <div className="bg-orange-50 text-orange-600 px-4 py-2.5 rounded-xl mb-4 text-xs flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
              </svg>
              Restam {attemptsRemaining} tentativa{attemptsRemaining !== 1 ? "s" : ""}
            </div>
          )}
